...
//       // resize on width
//       if (rectWidth / rectHeight > _preferredCropAspectRatio! && (oldRatio != null && oldRatio < _preferredCropAspectRatio!)) {
//         final w = rectHeight * _preferredCropAspectRatio!;
//         newCrop = Rect.fromLTWH(_rect.value.center.dx - w / 2, _rect.value.top, w, _rect.value.height);
//       } else {
//         // otherwise, resize on height
//         final h = rectWidth / _preferredCropAspectRatio!;
//         newCrop = Rect.fromLTWH(_rect.value.left, _rect.value.center.dy - h / 2, _rect.value.width, h);
//       }
//     }
//
//     // if new crop is bigger than available space, block to maximum size and avoid out of bounds
//     if (newCrop.width > _layout.width) {
//       final h = _layout.width / (_preferredCropAspectRatio ?? (rectWidth / rectHeight));
...
//       final w = _layout.height / (_preferredCropAspectRatio ?? (rectWidth / rectHeight));
//       newCrop = Rect.fromLTWH(
...
//     top = (top ?? _rect.value.top).clamp(0, max(0.0, _rect.value.bottom - minRectSize));
//     left = (left ?? _rect.value.left).clamp(0, max(0.0, _rect.value.right - minRectSize));
//     right = (right ?? _rect.value.right).clamp(_rect.value.left + minRectSize, _layout.width);
//     bottom = (bottom ?? _rect.value.bottom).clamp(_rect.value.top + minRectSize, _layout.height);
//
